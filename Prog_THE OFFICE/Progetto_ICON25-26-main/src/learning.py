...
def preprocessing_dataset(df):
    ...
    # Forziamo il dataset a mantenere SOLO le feature utili all'addestramento.
    # Tutto il resto (titoli, riassunti, date) verrà eliminato in automatico.
    feature_da_mantenere = ['Season', 'Episode', 'Director', 'Writers', 'Viewership', 'Votes', 'Ratings']
    
    colonne_da_eliminare = [col for col in df.columns if col not in feature_da_mantenere]
    df = df.drop(columns=colonne_da_eliminare, errors='ignore')

    # --- 2. DISCRETIZZAZIONE TARGET  ---
    # Creiamo la colonna 'Verdict' partendo dalla colonna continua 'Ratings'
    bins = [0, 7.8, 8.5, 10]
    labels = ['Scarso', 'Buono', 'Eccellente']
    
    df['Verdict'] = pd.cut(df['Ratings'], bins=bins, labels=labels)
    
    # Rimuoviamo il voto esatto per non viziare l'addestramento
    df = df.drop(columns=['Ratings'], errors='ignore')

    # --- 3. LABEL ENCODER  ---
    # Convertiamo i nomi di registi, sceneggiatori e le 3 etichette del Verdict in numeri
    ...
    # --- 4. DIVISIONE IN FEATURE (X) E TARGET (y) ---
